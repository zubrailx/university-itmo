...
import ru.nkulakov.exceptions.RecursiveCallException;
...
public class ExecuteScriptCommand extends Command {
     ...
     */
    public ExecuteScriptCommand(UserIO userIO, Script script) {
    ...
    public void execute(String[] arguments, InvocationStatus invocationEnum, PrintStream printStream, UserData userData, Lock locker) throws CannotExecuteCommandException {
        if (invocationEnum.equals(InvocationStatus.CLIENT)) {
            ...
            try {
                ...
                CommandInvokerImpl commandInvoker = new CommandInvokerImpl(userIO, dragonFieldsReader, script);
            ...
            printStream.println("The file the script executing: " + this.getResult().get(0));
            Object[] arr = result.toArray();
            arr = Arrays.copyOfRange(arr, 1, arr.length);
            CommandContainer[] containerArray = Arrays.copyOf(arr, arr.length, CommandContainer[].class);

            CommandInvokerImpl commandInvoker = new CommandInvokerImpl(collectionManager, cdh, locker);
    ...
    public String getDescription() {
        return "executes commands that are in the file";
    }
