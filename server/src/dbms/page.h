...
// PAGE CONTAINER
fileoff_t dbms_container_create_close(struct dbms *dbms, fileoff_t prev,
                                      pageoff_t size);
fileoff_t dbms_container_create(struct dbms *dbms, pageoff_t size, fileoff_t prev,
                                struct page_container **pc_ptr_out);

struct page_container *dbms_container_open(const struct dbms *dbms, fileoff_t pc_loc);

void dbms_container_close(struct page_container **page_ptr, fileoff_t page_start,
                          struct dbms *dbms);
bool dbms_container_find(const struct dbms *dbms, pageoff_t size, fileoff_t *loc_out,
                         pageoff_t *off_out);
