...
public class ReplaceIfLowerCommand implements CommandWithArguments {
    /**
     ...
     */
    public ReplaceIfLowerCommand(CollectionManager collectionManager, UserIO userIO) {
    ...
    /**
     * Метод, исполняющий команду. Если поля указанные значения полей меньше, то они изменятся, иначе нет. В случае некорректного ввода высветится ошибка или предупреждение.
     */
    @Override
    public void execute() {
        try {
            if (collectionManager.containsKey(Integer.parseInt(commandArguments[0]))) {
                ...
                if (str.length == 1) {
                    collectionManager.replaceIfLower(Integer.parseInt(commandArguments[0]), str[0], "");
                } else {
                    collectionManager.replaceIfLower(Integer.parseInt(commandArguments[0]), str[0], str[1]);
                }
